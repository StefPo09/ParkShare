...
export default function AddSpotPage() {
  ...
  // Stare imagine (începe de la null ca în AddCarPage)
  const [spotImage, setSpotImage] = useState<string | null>(null);
  const [isPhotoModalOpen, setIsPhotoModalOpen] = useState(false);
  const [isInfoModalOpen, setIsInfoModalOpen] = useState(false);
  const [isSuccessModalOpen, setIsSuccessModalOpen] = useState(false);

  // Stările obiectului "values" pornesc goale/default pentru adăugare
  ...
  const canSubmit =
      ...
      values.startHour.trim().length > 0 &&
      values.endHour.trim().length > 0 &&
      ...
      <div className="min-h-screen bg-[#dfeef0] px-0 py-0 dark:bg-[#011b1b] relative">
        <div className="mx-auto flex h-screen w-full max-w-107.5 flex-col overflow-hidden bg-[radial-gradient(circle_at_top,rgba(255,255,255,0.24),transparent_48%)] bg-[#dfeef0] text-[#121212] shadow-[0_25px_50px_rgba(15,32,35,0.12)] transition-colors duration-300 dark:bg-[radial-gradient(circle_at_top,rgba(255,255,255,0.04),transparent_36%)] dark:bg-[#011b1b] dark:text-white">
          ...
          <main className="flex-1 px-4 pt-4 overflow-y-auto space-y-4 pb-24 no-scrollbar">
            ...
            {/* Submit Button (Adăugare cu protecție disabled ca în AddCar) */}
            <div className="px-2 pt-4">
              <button
                  type="button"
                  disabled={!canSubmit}
                  onClick={handleAddSpotSubmit}
                  className="flex w-full cursor-pointer items-center justify-center rounded-2xl bg-[#0f4c81] px-5 py-3.5 text-base font-semibold text-white shadow-[0_16px_28px_rgba(15,76,129,0.28)] transition hover:bg-[#0c3e67] disabled:cursor-not-allowed disabled:bg-[#0f4c81]/45 disabled:shadow-none hover:scale-[1.01] active:scale-[0.99]"
              >
                Add Spot
              </button>
            </div>
