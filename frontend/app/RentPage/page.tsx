  ...
  return (
    ...
    >
      ...
      <div className="mx-auto flex h-screen w-full max-w-107.5 flex-col overflow-hidden bg-[#dfeef0] text-[#121212] shadow-[0_25px_50px_rgba(15,32,35,0.12)] transition-colors duration-300 dark:bg-[#011b1b] dark:text-white">
        ...
        {/* Header Navigation */}
        <header className="flex items-center justify-between px-5 pt-5 pb-2 z-10 bg-[#dfeef0] dark:bg-[#011b1b]">
          <button
        ...
        <main className="relative flex-1 bg-[#e8e8e8] dark:bg-[#121c1a] overflow-hidden transform-gpu">
          {isLoaded ? (
            <GoogleMap
          ...
          {selectedSpot && (
            <div
              ...
              className="absolute bottom-4 left-3 right-3 z-40 rounded-[32px] bg-[#cde8e8] dark:bg-[#0c2e2b] p-4 shadow-2xl transition-all duration-300 border border-black/5 dark:border-white/10"
            >
              {/* Drag Handle Indicator */}
              <div className="flex justify-center pb-2 cursor-grab active:cursor-grabbing">
                <div className="h-1.5 w-14 rounded-full bg-[#8baab0] dark:bg-[#204a44]" />
              </div>

              {/* Spot Image Preview with Pagination Dots */}
              <div className="relative h-44 w-full overflow-hidden rounded-[22px] bg-slate-200 shadow-inner">
                <Image
              ...
              <div className="mt-3 px-1">
                <h3 className="text-[20px] font-bold text-[#1a4a58] dark:text-[#a0ece0] tracking-tight">
                  {selectedSpot.address}
                </h3>

                <div className="mt-1 flex items-center gap-4 text-[15px] font-medium text-[#52737c] dark:text-[#74928d]">
                  <div className="flex items-center gap-1">
                    <Clock className="h-4 w-4 stroke-[2]" />
                    <span>{selectedSpot.availability || '14:00-18:00'}</span>
                  </div>
                  <div className="flex items-center gap-1">
                    <MapPin className="h-4 w-4 stroke-[2]" />
                    <span>{selectedSpot.distance || '100 meters'}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-3 flex items-center justify-between px-1">
                <div className="flex items-baseline gap-1">
                  <span className="text-[28px] font-bold text-[#143d49] dark:text-white">
                    ${selectedSpot.price}
                  </span>
                  <span className="text-[16px] font-medium text-[#52737c] dark:text-[#74928d]">
                    / hour
                  </span>
                </div>

                <button
                  onClick={() => router.push(`${ROUTES.RENT}?spot=${selectedSpot.id}`)}
                  className="rounded-[18px] bg-[#0c4a75] px-8 py-2.5 text-[22px] font-black tracking-wider text-white shadow-lg transition hover:bg-[#0a3c5f] active:scale-95 dark:bg-[#155b8a]"
                >
                  PAY
                </button>
              </div>
              ...
              <div className="mt-4 border-t border-black/5 dark:border-white/10 pt-3 px-1">
                <h4 className="text-[17px] font-bold text-[#1a4a58] dark:text-[#a0ece0]">
                  Location name
                </h4>
              </div>
            </div>
